...
int main(int argc, char** argv) {
    ...
    //Declare Variables
    bool boolPly=false;                 //bool to decide if going to play
    int gameCnt;                        //counter for games played
    float favNum,favSqua;               //favorite number/fav number squared
    char play;                          //input variable to play or not
    ...
    ofstream out;                       //file stream output
    string fileIn;                      
    char fileOut[]="Games.dat";         //name of file to output games to
    fileIn="FavNum.dat";                //name of file to input favorite number
    in.open(fileIn.c_str());            //open input file
    out.open(fileOut);                  //open output file
    //Introduction
    in>>favNum;                         //read in favorite number from file
    cout<<"Hello my favorite number is "<<favNum<<endl;
    favSqua=pow(favNum,2);              //calculate favorite number squared
    cout<<"My favorite number squared is "<<favSqua<<endl;
    //Initialize variables
    gameCnt=0;                          //initialize game counter to 0
    cout<<"Would you like to play a game of Blac  Jack? [Y/N]"<<endl;
    cin>>play;                              //determine if they want to play
    if(play=='Y'||play=='y'){cout<<"Lets play!"<<endl;boolPly=true;}//convert to
    else if (play=='N'||play=='n'){cout<<"You chose not to play"    //bool
            <<endl;boolPly=false;}
    else{ cout<<"Invalid input, ending program"<<endl; return 0;}//validate user
    while(boolPly){                 //start of game/testing bool //input
        plScore=0;                  //initialize scores to 0
        cpScore=0;                  //initialize scores to 0
        ...
        if(plSuit1==0){                   //If the suit of card 1 is hearts 
            switch(plCard1){              //using a switch instead of if/elseif
                case 2:cout<<"Your first card is 2 of Hearts"<<endl;break;
                case 3:cout<<"Your first card is 3 of Hearts"<<endl;break;
                case 4:cout<<"Your first card is 4 of Hearts"<<endl;break;
                case 5:cout<<"Your first card is 5 of Hearts"<<endl;break;
                case 6:cout<<"Your first card is 7 of Hearts"<<endl;break;
                case 8:cout<<"Your first card is 8 of Hearts"<<endl;break;
                case 9:cout<<"Your first card is 9 of Hearts"<<endl;break;
                case 10:cout<<"Your first card is 10 of Hearts"<<endl;break;
                case 11:cout<<"Your first card is Jack of Hearts"<<endl;break;
                case 12:cout<<"Your first card is Queen of Hearts"<<endl;
                break;
                case 13:cout<<"Your first card is King of Hearts"<<endl;break;
                case 14:cout<<"Your first card is Ace of Hearts"<<endl;break;
        ...
        //1111112222222233333333444444445555555566666666777777778888888899999999
        //Determine winner
        gameCnt++;               //add 1 to counter for number of games played
        ...
        cout<<"Would you like to play another game of Black Jack? [Y/N]"
                    <<endl;
        cin>>play;              //input choice to play again           //convert
        if(play=='Y'||play=='y'){cout<<"Lets play!"<<endl;boolPly=true;}//to
        else if (play=='N'||play=='n'){cout<<"You chose not to play"    //bool
                <<endl;boolPly=false;}
        else{ cout<<"Invalid input, ending program"<<endl; return 0;}//validate
    }                                                               //user input
    //Output to file
    out<<"You have played "<<gameCnt<<" game(s)"<<endl;
    //close the files
